...
export function Properties() {
    ...
    return (
        <Transition className="px-4 my-8 md:py-32 md:px-40">
            <div id="propierties" className="grid grid-cols-2 gap-4 md:grid-cols-4">
                {dataFilteredHouses.map(({id, price, location, bedrooms, bathroom, image, star, meters}) => (
                    <Link key={id} href={`/properties/${id}`} className="shadow-light hover:shadow-xl rounded-2xl transition-all duration-300 cursor-pointer">
                        <div className="relative -z-[1]">
                            <div className="relative">
                                <div className="absolute flex items-center px-2 py-1 rounded-lg bg-white top-2 right-2 text-secondary">
                                    <LiaStarSolid/>
                                    <span className="nl-1 font-semibold">{star}</span>
                                </div>
                                ...
                                <div className="px-3 py-5">
                                    ...
                                    <p className="fond-semibold">{formatPrice(price)}</p>
