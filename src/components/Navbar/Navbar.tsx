        ...
        <AnimatePresence>
            { isScrolling ? (
                <motion.nav key={1}
                    ...
                    animate="animate"
                    exit="exit"
                    className="ml-auto mr-auto md-fixed z-[9999] right-0 left-0 px-6 py-3 text-white bg-grey-400/40 top-10 rounder-3xl backdrop-blur w-fit"
                >  
            ...
            <div className={`${openMobileMenu ? 'absolute z-[1] left-0 top-20 top-20 bg-white r-0 w-full px-4 py-4':'hidden'} gap-5 md:flex`}>
                {dataHeader.map(({id, name, link}) => (
